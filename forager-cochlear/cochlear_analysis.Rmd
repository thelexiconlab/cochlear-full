...
desired_order <- c("50", "100", "200","300")  # Replace with your actual group names

comp_sem_sim_across <- comb_sem_sim_comparison %>%
  mutate(dimension = factor(dimension, levels = desired_order)) %>%  # Convert to factor with specified order
...
best_models <- agg_model_sum_nLL %>%
  ...
  ungroup() %>%
   select(-fullname) %>% 
  select(Group,domain,struct,dimension,foraging_type,method,param1,param2,param3,sum_nLL) 
kable(best_models)
