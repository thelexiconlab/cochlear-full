...
def calculate_model(model, history_vars, switch_names, switch_vecs):
    ...
    # Initialize r1, r2 with default values
    ...
    if model == models[0] or model == models[4]:
        ...
        # Save results for the static model
        model_name.append('forage_static')
        model_results.append((
            beta_df,  
            beta_ds,  
            None,     # No switch betas
            None,     
            None,     # No phonological betas
            nll       
        ))
    ...
    if model == models[1] or model == models[4]:
        for i, switch_vec in enumerate(switch_vecs):
            # Ensure switch_names and switch_vecs are the same length
            if i >= len(switch_names):
                raise ValueError("Mismatch between switch_names and switch_vecs: index out of range.")

            # Identify indices for clusters and switches
            cluster_indices = [idx for idx, val in enumerate(switch_vec) if val == 0]
            switch_indices = [idx for idx, val in enumerate(switch_vec) if val == 1]

            # Filter variables for clusters
            def filter_vars(indices):
                return (
                    [history_vars[2][idx] for idx in indices],  # freql
                    [history_vars[3][idx] for idx in indices],  # freqh
                    [history_vars[0][idx] for idx in indices],  # siml
                    [history_vars[1][idx] for idx in indices],  # simh
                    [switch_vec[idx] for idx in indices]        # switchvals
                )

            # Extract data for clusters and switches
            cluster_data = filter_vars(cluster_indices)
            switch_data = filter_vars(switch_indices)

            # Function to optimize model parameters
            def optimize_model(data, label):
                r1, r2 = np.random.rand(), np.random.rand()
                beta_opt = minimize(forage.model_dynamic, [r1, r2], args=data).x
                beta_df, beta_ds = float(beta_opt[0]), float(beta_opt[1])
                return beta_df, beta_ds

            # Calculate betas separately for clusters and switches
            beta_df_0, beta_ds_0 = optimize_model(cluster_data, "Cluster")
            beta_df_1, beta_ds_1 = optimize_model(switch_data, "Switch")

            # Combine data for NLL calculation
            combined_data = (
                cluster_data[0] + switch_data[0],  # Combine freql
                cluster_data[1] + switch_data[1],  # Combine freqh
                cluster_data[2] + switch_data[2],  # Combine siml
                cluster_data[3] + switch_data[3],  # Combine simh
                cluster_data[4] + switch_data[4]   # Combine switchvals
            )

            # Optimize for combined data
            r1, r2 = np.random.rand(), np.random.rand()
            beta_opt = minimize(forage.model_dynamic, [r1, r2], args=combined_data).x
            beta_df_combined, beta_ds_combined = float(beta_opt[0]), float(beta_opt[1])
            nll, nll_vec = forage.model_dynamic_report([beta_df_combined, beta_ds_combined], *combined_data)

            # Print debug info
    

            # Save results for this switch
            model_name.append(f'forage_dynamic_{switch_names[i]}')
            model_results.append((beta_df_0, beta_ds_0, beta_df_1, beta_ds_1, nll))

    # STATIC PHONOLOGICAL MODEL
    ...
    if model == models[3] or model == models[4]:
        for i, switch_vec in enumerate(switch_vecs):
            ...
            # Filter variables for clusters and switches
            def filter_vars(indices):
                return (
                    [history_vars[2][idx] for idx in indices],  # freql
                    [history_vars[3][idx] for idx in indices],  # freqh
                    [history_vars[0][idx] for idx in indices],  # siml
                    [history_vars[1][idx] for idx in indices],  # simh
                    [history_vars[4][idx] for idx in indices],  # phonl
                    [history_vars[5][idx] for idx in indices],  # phonh
                    [switch_vec[idx] for idx in indices]        # switchvals
                )
            ...
            # Function to optimize the phonological model (common for all variations)
            def optimize_model(data, model_type):
                r1, r2, r3 = np.random.rand(), np.random.rand(), np.random.rand()
                beta_opt = minimize(forage.model_dynamic_phon, [r1, r2, r3], args=data + (model_type,)).x
                beta_df, beta_ds, beta_dp = float(beta_opt[0]), float(beta_opt[1]), float(beta_opt[2])
                return beta_df, beta_ds, beta_dp

            ### Global Dynamic Phonological Model
            # Optimize betas for clusters and switches (Global)
            beta_df_0, beta_ds_0, beta_dp_0 = optimize_model(cluster_data, "global")
            beta_df_1, beta_ds_1, beta_dp_1 = optimize_model(switch_data, "global")

            # Combine data for NLL calculation (Global)
            combined_data = (
                cluster_data[0] + switch_data[0],  # Combine freql
                cluster_data[1] + switch_data[1],  # Combine freqh
                cluster_data[2] + switch_data[2],  # Combine siml
                cluster_data[3] + switch_data[3],  # Combine simh
                cluster_data[4] + switch_data[4],  # Combine phonl
                cluster_data[5] + switch_data[5],  # Combine phonh
                cluster_data[6] + switch_data[6]   # Combine switchvals
            )

            # Calculate NLL for the combined data (Global)
            r1, r2, r3 = np.random.rand(), np.random.rand(), np.random.rand()
            beta_opt = minimize(forage.model_dynamic_phon, [r1, r2, r3], args=combined_data + ("combined",)).x
            beta_df_combined, beta_ds_combined, beta_dp_combined = float(beta_opt[0]), float(beta_opt[1]), float(beta_opt[2])
            nll, nll_vec = forage.model_dynamic_phon_report([beta_df_combined, beta_ds_combined, beta_dp_combined], *combined_data)

            # Save results for Global Dynamic Model
            model_name.append(f'forage_phonologicaldynamicglobal_{switch_names[i]}')
            model_results.append((
                beta_df_0, beta_ds_0, beta_dp_0,  # Cluster Phonological Betas
                beta_df_1, beta_ds_1, beta_dp_1,  # Switch Phonological Betas
                nll  # Combined NLL
            ))

            ### Local Dynamic Phonological Model
            # Optimize betas for clusters and switches (Local)
            beta_df_0, beta_ds_0, beta_dp_0 = optimize_model(cluster_data, "local")
            beta_df_1, beta_ds_1, beta_dp_1 = optimize_model(switch_data, "local")

            # Combine data for NLL calculation (Local)
            combined_data = (
                cluster_data[0] + switch_data[0],  # Combine freql
                cluster_data[1] + switch_data[1],  # Combine freqh
                cluster_data[2] + switch_data[2],  # Combine siml
                cluster_data[3] + switch_data[3],  # Combine simh
                cluster_data[4] + switch_data[4],  # Combine phon_freq
                cluster_data[5] + switch_data[5],  # Combine phon_sim
                cluster_data[6] + switch_data[6]   # Combine switchvals
            )

            # Calculate NLL for the combined data (Local)
            r1, r2, r3 = np.random.rand(), np.random.rand(), np.random.rand()
            beta_opt = minimize(forage.model_dynamic_phon, [r1, r2, r3], args=combined_data + ("combined",)).x
            beta_df_combined, beta_ds_combined, beta_dp_combined = float(beta_opt[0]), float(beta_opt[1]), float(beta_opt[2])
            nll, nll_vec = forage.model_dynamic_phon_report([beta_df_combined, beta_ds_combined, beta_dp_combined], *combined_data)

            # Save results for Local Dynamic Model
            model_name.append(f'forage_phonologicaldynamiclocal_{switch_names[i]}')
            model_results.append((
                beta_df_0, beta_ds_0, beta_dp_0,  # Cluster Phonological Betas
                beta_df_1, beta_ds_1, beta_dp_1,  # Switch Phonological Betas
                nll  # Combined NLL
            ...
            # Optimize betas for clusters and switches (Switch)
            beta_df_0, beta_ds_0, beta_dp_0 = optimize_model(cluster_data, "switch")
            beta_df_1, beta_ds_1, beta_dp_1 = optimize_model(switch_data, "switch")

            # Combine data for NLL calculation (Switch)
            combined_data = (
                cluster_data[0] + switch_data[0],  # Combine freql
                cluster_data[1] + switch_data[1],  # Combine freqh
                cluster_data[2] + switch_data[2],  # Combine siml
            ...
            # Calculate NLL for the combined data (Switch)
            r1, r2, r3 = np.random.rand(), np.random.rand(), np.random.rand()
            beta_opt = minimize(forage.model_dynamic_phon, [r1, r2, r3], args=combined_data + ("combined",)).x
            beta_df_combined, beta_ds_combined, beta_dp_combined = float(beta_opt[0]), float(beta_opt[1]), float(beta_opt[2])
            nll, nll_vec = forage.model_dynamic_phon_report([beta_df_combined, beta_ds_combined, beta_dp_combined], *combined_data)

            # Save results for Switch Dynamic Model
            model_name.append(f'forage_phonologicaldynamicswitch_{switch_names[i]}')
            model_results.append((
                beta_df_0, beta_ds_0, beta_dp_0,  # Cluster Phonological Betas
                beta_df_1, beta_ds_1, beta_dp_1,  # Switch Phonological Betas
                nll  # Combined NLL
    ...
    model_results.append((0, 0, nll_baseline))
...
def run_model(data, model_type, switch_type, domain, speech, dimension, history_vars_list, switch_names_list, switch_vecs_list, alpha=None):
    ...
    # Define all possible model types and switch types
    all_models = ['static', 'dynamic', 'pstatic', 'pdynamic']
    all_switches = ['simdrop', 'multimodal', 'norms_associative', 'norms_categorical', 'delta', 'multimodaldelta']

    # Determine which model types and switch types to use
    if model_type == "all":
        model_types = all_models  # Use all available models
    else:
        model_types = [model_type]  # Use the specified model type

    if switch_type == "all":
        switch_types = all_switches  # Use all available switches
    else:
        switch_types = [switch_type]  # Use the specified switch type

    # Run through each combination of model type and switch type
    for model_type in model_types:
        for switch_type in switch_types:
            print(f"\nRunning Model: {model_type} with Switch: {switch_type}")

            # Run through each fluency list in the dataset
            for i, (subj, fl_list) in enumerate(tqdm(data)):
                print(f"\nRunning Model for Subject {subj}")
                import time
                ...
                for j, model in enumerate(model_names):
                    ...
                    # Get the result for this model
                    model_result = model_results[j]
                    print(f"\nModel Result for {model}: {model_result}")  # Debugging print statement to check model results

                    # Handle random baseline
                    if model == 'forage_random_baseline':
                        print("Handling 'forage_random_baseline'")
                        model_dict['Beta_Frequency_Cluster'] = model_result[0]  # Cluster Beta_Frequency
                        model_dict['Beta_Semantic_Cluster'] = model_result[1]   # Cluster Beta_Semantic
                        model_dict['Beta_Frequency_Switch'] = None  # No switch for baseline
                        model_dict['Beta_Semantic_Switch'] = None   # No switch for baseline
                        model_dict['Beta_Phonological_Cluster'] = None  # No phonological for baseline
                        model_dict['Beta_Phonological_Switch'] = None   # No phonological for baseline
                        model_dict['Negative_Log_Likelihood_Optimized'] = model_result[-1]  # NLL for baseline

                    # Handle dynamic models
                    elif model.lower().startswith("forage_dynamic"):
                        print(f"Handling dynamic model: {model}")
                        model_dict['Beta_Frequency_Cluster'] = model_result[0]  # Cluster Beta_Frequency
                        model_dict['Beta_Semantic_Cluster'] = model_result[1]   # Cluster Beta_Semantic
                        model_dict['Beta_Frequency_Switch'] = model_result[2]    # Switch Beta_Frequency
                        model_dict['Beta_Semantic_Switch'] = model_result[3]     # Switch Beta_Semantic
                        model_dict['Beta_Phonological_Cluster'] = None  # No phonological for dynamic model
                        model_dict['Beta_Phonological_Switch'] = None   # No phonological for dynamic model
                        model_dict['Negative_Log_Likelihood_Optimized'] = model_result[-1]  # NLL for dynamic model

                    # Handle dynamic phonological models
                    elif model.lower().startswith("forage_phonological"):
                        print("Handling 'forage_phonologicaldynamic' model")
                        model_dict['Beta_Frequency_Cluster'] = model_result[0]  # Cluster Beta_Frequency
                        model_dict['Beta_Semantic_Cluster'] = model_result[1]   # Cluster Beta_Semantic
                        model_dict['Beta_Frequency_Switch'] = model_result[2]    # Switch Beta_Frequency
                        model_dict['Beta_Semantic_Switch'] = model_result[3]     # Switch Beta_Semantic
                        model_dict['Beta_Phonological_Cluster'] = model_result[4]  # Cluster Beta_Phonological
                        model_dict['Beta_Phonological_Switch'] = model_result[5]   # Switch Beta_Phonological
                        model_dict['Negative_Log_Likelihood_Optimized'] = model_result[-1]  # NLL for phonological dynamic model

                    # Handle static models
                    elif model.lower().startswith("forage_static"):
                        print("Handling 'forage_static' model")
                        model_dict['Beta_Frequency_Cluster'] = model_result[0]  # Cluster Beta_Frequency
                        model_dict['Beta_Semantic_Cluster'] = model_result[1]   # Cluster Beta_Semantic
                        model_dict['Beta_Frequency_Switch'] = None    # Switch Beta_Frequency
                        model_dict['Beta_Semantic_Switch'] = None    # Switch Beta_Semantic
                        model_dict['Beta_Phonological_Cluster'] = None  # Cluster Beta_Phonological
                        model_dict['Beta_Phonological_Switch'] = None   # Switch Beta_Phonological
                        model_dict['Negative_Log_Likelihood_Optimized'] = model_result[-1]  # NLL for phonological dynamic model

                    else:  
                        print("Handling 'forage_pstatic' model")
                        model_dict['Beta_Frequency_Cluster'] = model_result[0]  # Cluster Beta_Frequency
                        model_dict['Beta_Semantic_Cluster'] = model_result[1]   # Cluster Beta_Semantic
                        model_dict['Beta_Frequency_Switch'] = None    # Switch Beta_Frequency
                        model_dict['Beta_Semantic_Switch'] = None    # Switch Beta_Semantic
                        model_dict['Beta_Phonological_Cluster'] = model_result[2]  # Cluster Beta_Phonological
                        model_dict['Beta_Phonological_Switch'] = None   # Switch Beta_Phonological
                        model_dict['Negative_Log_Likelihood_Optimized'] = model_result[-1]  # NLL for phonological dynamic model
    ...
    forager_results_df = pd.DataFrame(forager_results)

    # Optionally, save to CSV
    forager_results_df.to_csv("forager_model_results.csv", index=False)

    # Return the DataFrame
    return forager_results_df
