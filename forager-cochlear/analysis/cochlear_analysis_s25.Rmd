...
prop.test(x = c(nh_0.9, ci_0.9), n = c(nh_foods_total, ci_foods_total), correct = FALSE)
#sig more for CI
...
#comparing multimodal alphas across domains, collapsing across groups
...
animals_switch_alpha <- 3    
foods_switch_alpha <- 19  
...
prop.test(x = c(animals_switch_alpha,foods_switch_alpha), n = c(animals_total, foods_total), correct = FALSE)
#NHs marg more likely to have a lower alpha
...
#do groups differ in use of phon models, collapsing across domains
...
phonemes<-rbind(animals_phonemes,foods_phonemes)%>% rename(Subject="ID") %>%left_join(participant_groupings)%>%filter(!(Subject %in% c("CAN-667","SFZ-149","SOA-317","CDH-754","SGO-159")))%>%filter(Word1 != Word2) #filter out atypical subjects and consecutive repeats
...
#when looking at phon sim this way, are there group x domain differences in consec shared phoneme length
phoneme_model<-lmer(data = phonemes, Shared_Cont_Phonemes ~ Group*domain + (1|Subject))
summary(phoneme_model)
#large ME for domain but no group effect - but we aren't separating out clusters/switches

#what about when you remove shared stems
phoneme_model_nostems<-lmer(data = phonemes %>%filter(Shared_Cont_Phonemes<4), Shared_Cont_Phonemes ~ Group*domain+(1|Subject))
summary(phoneme_model_nostems)
#didn't make a difference

ggplot(data=phoneme_indv,aes(y=avg_shared_length, x = total, group = Group, color = Group)) + facet_wrap(~domain)+
  theme_few() + geom_point()+
  geom_smooth(method = "lm", se = TRUE,size=1) +
  theme(legend.position="top",
    aspect.ratio = 1,  
    axis.text.x = element_text( hjust = 1, face = "bold"))+scale_colour_paletteer_d("Manu::Kereru")+labs(x = "fluency score", y = "avg shared consecutive phonemes")
...
demos_clean<-demos %>% select(ID,Group,Gender,NIH_Race,CAYRS2,Eti,FitMOS,UseYRS2) %>%filter(!(ID %in% c("754", "159")))%>%
  ...
  Age = CAYRS2,
  Etiology = Eti,
  `Age of Implantation (in Months)` = FitMOS,
  `Length of Use` = UseYRS2)
...
#check counts
subjects_demos <- unique(demos_combined$Subject)
subjects_bic <-unique(substring(best_blended_delta_bic$Subject, 5))
# Subjects in model df but not in demos
missing_in_demos <- setdiff(subjects_bic, subjects_demos)
cat("\nSubjects in best_blended_delta_bic but not in demos_combined:\n")
print(missing_in_demos)


demo_table <- demos_combined %>%
  select(-Subject) %>%
  tbl_summary(
    by = Group,
