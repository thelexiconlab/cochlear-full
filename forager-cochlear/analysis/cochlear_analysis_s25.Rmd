...
```{r}
#Getting N
just_fluency<-full_indv_data %>%select(Subject,Group,domain,fluency_score) %>%rename("N"="fluency_score") %>%na.omit() %>%unique()%>%  filter(!(Subject %in% c("CAN-667","CAU-667","SFZ-149","SOA-317","CDH-754","SGO-159")))
...
full_model_data_parameters<-full_model_data %>%mutate(parameter_link=paste(model_type,method,sep="_"))%>% filter(!(Subject %in% c("CAN-667","CAU-667","SFZ-149","SOA-317","CDH-754","SGO-159"))) #exclude missing demos and atypical subjs
...
phonemes<-rbind(animals_phonemes,foods_phonemes)%>% rename(Subject="ID") %>%left_join(participant_groupings)%>%filter(!(Subject %in% c("CAN-667","SFZ-149","SOA-317","CDH-754","SGO-159")))%>%filter(Word1 != Word2) #filter out atypical/missing demo subjects 

#get list of words w/ shared stems
phonemes_stem<-phonemes%>%
  filter(Shared_Cont_Phonemes > 2) %>%
  arrange(Subject, Group, domain) %>%
  filter(lag(Word2) != Word1 | is.na(lag(Word2))) %>%  # keep only if not repeated
  select(Subject, Group, domain, Word1, Word2)%>%
...
#  anti_join(
#    comb_phonemes %>% select(Subject, entry),
 #   by = c("Subject", "entry"))
...
# NOT INCL FLUENCY SCORE AS COVARIATE
#avg cluster size
#number of items produced as a covariate
domain_cluster_size_nofs_lm = lm(data = best_clusters, Cluster_Size_mean ~ Group * domain)
summary(domain_cluster_size_nofs_lm)

#number of switches
domain_switch_numb_nofs_lm = lm(data = best_clusters, Number_of_Switches ~Group * domain)
summary(domain_switch_numb_nofs_lm)
#ME of fluency score
