...
#switch designations
#A LOT OF DATA - TAKES ~2hr TO RUN
full_switch_data <- combine_model_data("switch_results.csv") %>%select(-file_type)%>%left_join(participant_groupings)%>% separate(Switch_Method, into = c("method", "param1", "param2", "param3"), sep = "_", fill = "right")%>%mutate(full_method=paste(method,param1,param2,param3,sep="_"))
...
plots_lexical$emmip_plot[[2]]
plots_lexical$emmip_plot[[3]]
...
## Best Model for Each Specific Group x Domain
```{r}
...
```{r}
#filtering to only include best switch method for each domain
...
switches_recoded <- best_switches %>%
  ...
  left_join(items_lexical_data, by = c("Subject", "Fluency_Item", "domain", "cattype", "row_id")) %>%
  rename(
...
switch_results <- switches_recoded  %>%
  ...
  nest() %>%                                         
 mutate(
    model = map(data, ~ lmer(value ~ Group*domain*Switch+(1|Subject), data = .)) 
  )
...
#phonology
#emmeans(switch_results$model[[2]],pairwise~domain*Switch,simple="Switch",pbkrtest.limit = 35657)
```
...
wiac_cluster_plots <- map(plot_configs, ~ {
  switches_recoded %>%
    filter(cue == .x$cue) %>%
    mutate(
      Switch = ifelse(.x$cue == "frequency" & is.na(Switch), "Cluster Transition", Switch),
      Switch = factor(Switch, levels = c("Within Cluster Transition", "Switch Trial"))
    ) %>%
    group_by(domain, Switch) %>%
    tidyboot_mean(get(.x$value_col), nboot = 1000, na.rm = TRUE) %>%
    ggplot(aes(fill = Switch, y = empirical_stat, x = Group)) + facet_wrap(~domain)+
    geom_bar(stat = "identity", position = "dodge") +
...
wiac_cluster_plots[[1]] + wiac_cluster_plots[[2]] + wiac_cluster_plots[[3]]
