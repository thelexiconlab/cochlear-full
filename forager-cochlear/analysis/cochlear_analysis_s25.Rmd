...
switch_prop <- best_blended_delta_bic_analysis %>%
  ...
  group_by(Group, domain) %>%
  count(Group, domain, param1, name = "n") %>%         # tallies by Group×domain×param1
  mutate(prop = n / sum(n)) %>%                        # % within each Group×domain
  ungroup()
...
alpha_implant <- best_blended_delta_bic_analysis %>%
  filter(Group == "CI") %>%
  select(Subject, Group, domain, alpha) %>%              # keep Group & domain
  rename(ID = Subject) %>%
  mutate(
