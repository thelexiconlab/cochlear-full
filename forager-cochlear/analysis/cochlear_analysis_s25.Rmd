...
lowest_bic %>%
  ...
  facet_grid(domain ~ Group) +  # Correct facet_grid syntax
  xlim(-600, 0)
...
best_models <- full_model_data %>%mutate(Group = factor(Group, levels = c("NH","CI")))%>%
  filter(paste(Group,domain,everything) %in% paste(median_bic$Group, median_bic$domain, median_bic$everything)) %>%
