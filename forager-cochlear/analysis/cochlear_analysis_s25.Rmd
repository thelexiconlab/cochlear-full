...
#note these are raw entries
#read in phoneme data

animals_phonemes<-read.csv("../animals_phonemes_with_stems.csv") %>%rename("Subject"="ID") %>%mutate(domain="animals")
foods_phonemes<-read.csv("../foods_phonemes_with_stems.csv") %>%rename("Subject"="ID")%>%mutate(domain="foods")
phonemes<- rbind(animals_phonemes,foods_phonemes) %>% left_join(participant_groupings) %>%left_join(just_fluency)

#Looking at words with 4 shared continuous phonemes
phonemes_stem <- phonemes %>%
  group_by(Group, domain) %>%
  summarise(
    proportion_4plus = sum(Shared_Cont_Phonemes > 3, na.rm = TRUE) / n(),
    avg_shared_cont = mean(Shared_Cont_Phonemes, na.rm = TRUE),
    .groups = "drop")
phonemes_stem

#put into wide format, look at individual pairs?
#some kind of chart reflecting proportion? also need to do proportion testing probs
#think about way to break out into clusters vs switches, dealing w/ replacements, removed repetitions, etc
```
