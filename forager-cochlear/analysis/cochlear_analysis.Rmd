...
comb_table<-rbind(animals_group_fluency %>% mutate(domain="animals"),foods_group_fluency %>%mutate(domain="foods"))
kable(comb_table %>% select(Group, domain, fluency_mean, fluency_sd)) %>%
  kable_styling()
...
#Incl USE: Does semantic similarity differ by group x struct
...
emmeans(comb_psim_model,pairwise~Group*domain*Phonological_Similarity_mean,simple="Group")
...
emmeans(comb_semsim_model,pairwise~Group*Semantic_Similarity_mean,simple="Group")
...
emmeans(comb_freq_model,pairwise~Group*Frequency_Value_mean,simple="Group")
...
emmeans(comp_switch_lm,pairwise~Group*method,simple="Group")
...
emmeans(cluster_size_lm,pairwise~domain)
...
summary(switch_lm)
car::Anova(switch_lm)
emmeans(switch_lm,pairwise~domain)
...
```{r}
#check for reps in og and in processed data
#Try just running 657 
...
#Dropping inf values
#A ton are being dropped here- normal?
dropped_values <- agg_model_results %>%
...
#Betas for each subjects best model
...
best_clusters = comb_clustering %>% filter(Switch_Method==best_method) %>%
  summarise(
    cluster_mean = mean(Cluster_Size_mean),
    cluster_sd = sd(Cluster_Size_std),
    num_switches = mean(Number_of_Switches),
...
all_best_clusters = comb_clustering %>% filter(Switch_Method==best_method)

#When using the overall best method, does avg cluster mean differ by Group x domain
all_best_cluster_size_lm = lmer(data = all_best_clusters, Cluster_Size_mean ~ Group*domain + (1|Subject))
summary(all_best_cluster_size_lm)
car::Anova(all_best_cluster_size_lm)

#When using the best method, does avg numb switches differ by Group x domain
all_switch_numb_lm = lmer(data = all_best_clusters, Number_of_Switches ~ Group*domain + (1|Subject))
summary(all_switch_numb_lm)
car::Anova(all_switch_numb_lm)


all_best_cluster<-all_best_clusters %>%
  group_by(Group, domain) %>% 
...
all_best_switches<-all_best_clusters %>%
  group_by(Group, domain) %>% 
...
gridExtra::grid.arrange(all_best_cluster, all_best_switches, ncol=2)


#By subject betas for overall best model
best_type <- as.character(overall_best_model$full_model)
best_model_betas <- agg_model_results_clean %>%
  ...
  filter(as.character(full_model) == best_type)
...
#Beta testing - Best overall model
#Does use of semantic similarity differ across Groups x domains
best_beta_semantic_model = lm(data = best_beta_values %>% filter(beta == "Beta_Semantic"), value ~ Group*domain)
...
#Does use of frequency differ across Groups x domains
best_beta_freq_model = lm(data = best_beta_values %>% filter(beta == "Beta_Frequency"), value ~ Group*domain)
summary(best_beta_freq_model)
car::Anova(best_beta_freq_model)
emmeans(best_beta_freq_model,pairwise~Group*domain, simple="Group")

#Does use of phonological similarity differ across Groups x domains
best_beta_phon_model = lm(data = best_beta_values %>% filter(beta == "Beta_Phonological"), value ~ Group*domain)
...
best_nh_clusters = comb_clustering %>% filter(Group=="normal hearing") %>%filter(Switch_Method==nh_best_method) %>%
  summarise(
    cluster_mean = mean(Cluster_Size_mean),
    cluster_sd = sd(Cluster_Size_std),
    num_switches = mean(Number_of_Switches),
    sd_switches = sd(Number_of_Switches)) %>%
  mutate(Group="NH") 
best_ci_clusters = comb_clustering %>% filter(Group=="cochlear implant") %>%filter(Switch_Method==ci_best_method) %>% 
  summarise(
    cluster_mean = mean(Cluster_Size_mean),
    cluster_sd = sd(Cluster_Size_std),
    num_switches = mean(Number_of_Switches),
...
#Full participant data
full_nh_clusters = comb_clustering %>% filter(Group=="normal hearing")%>% filter(Switch_Method==nh_best_method)

full_ci_clusters = comb_clustering %>% filter(Group=="cochlear implant") %>%filter(Switch_Method==ci_best_method) 
