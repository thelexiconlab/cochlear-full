...
emmeans(animals_USE_sem_sim_lm,pairwise~Group*struct,simple="struct")
...
desired_order_dim <- c("50", "100", "200","300","512")  # Replace with your actual group names
desired_order_struct <- c("w2v", "s2v", "USE")
comp_USE_sem_sim_across <- comb_USE_sem_sim_comparison %>%
  mutate(
    dimension = factor(dimension, levels = desired_order_dim),  # Ensure this is correctly set
    struct = factor(struct, levels = desired_order_struct)        # Set the structure order
  ) %>%
