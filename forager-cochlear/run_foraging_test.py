...
def calculate_model(model, history_vars, switch_names, switch_vecs):
    ...
    if model == models[1] or model == models[4]:
        ...
        # Get indices for clusters and switches
            cluster_idx = [idx for idx, val in enumerate(switch_vec) if val == 0]
            switch_idx = [idx for idx, val in enumerate(switch_vec) if val == 1]
        
        # Random initializations
            r1_cluster, r2_cluster = np.random.rand(), np.random.rand()
            r1_switch, r2_switch = np.random.rand(), np.random.rand()

        # cluster trials
            if len(cluster_idx) > 0:
                v_cluster = minimize(
                    forage.model_dynamic, [r1_cluster, r2_cluster], 
                    args=(
                    [history_vars[2][idx] for idx in cluster_idx],  # List comprehension fix
                    [history_vars[3][idx] for idx in cluster_idx],
                    [history_vars[0][idx] for idx in cluster_idx],
                    [history_vars[1][idx] for idx in cluster_idx],
                    [switch_vec[idx] for idx in cluster_idx])).x
                beta_df_cluster = float(v_cluster[0])  
                beta_ds_cluster = float(v_cluster[1]) 
                 
            else:
                beta_df_cluster, beta_ds_cluster = np.nan, np.nan  

            # switch trials
            if len(switch_idx) > 0:
                v_switch = minimize(
                    forage.model_dynamic, [r1_switch, r2_switch], 
                    args=(
                    [history_vars[2][idx] for idx in switch_idx],  
                    [history_vars[3][idx] for idx in switch_idx],
                    [history_vars[0][idx] for idx in switch_idx],
                    [history_vars[1][idx] for idx in switch_idx],
                    [switch_vec[idx] for idx in switch_idx])).x
                
                beta_df_switch = float(v_switch[0])  
                beta_ds_switch = float(v_switch[1])  
            else:
                beta_df_switch, beta_ds_switch = np.nan, np.nan  

            
            # Compute NLL using all data
            nll, nll_vec = forage.model_dynamic_report(
                [beta_df_cluster, beta_ds_cluster, beta_df_switch, beta_ds_switch], 
                history_vars[2], history_vars[3],
                history_vars[0], history_vars[1],
                switch_vec)
            
            # Store results
            model_name.append('forage_dynamic_' + switch_names[i])
            model_results.append((beta_df_cluster, beta_ds_cluster, beta_df_switch, beta_ds_switch, nll, nll_vec))
    ...
    if model == models[3] or model == models[4]:
        ...
        # Random initializations for cluster
            r1_cluster, r2_cluster, r3_cluster = np.random.rand(), np.random.rand(), np.random.rand()
        # Random initializations for switch
            r1_switch, r2_switch, r3_switch = np.random.rand(), np.random.rand(), np.random.rand()

        # cluster trials
            if len(cluster_idx) > 0:
                v_cluster = minimize(
                    forage.model_dynamic_phon, 
                    [r1_cluster, r2_cluster, r3_cluster], 
                    args=(
                        [history_vars[2][idx] for idx in cluster_idx],  
                        [history_vars[3][idx] for idx in cluster_idx],
                        [history_vars[0][idx] for idx in cluster_idx],
                        [history_vars[1][idx] for idx in cluster_idx],
                        [history_vars[4][idx] for idx in cluster_idx],
                        [history_vars[5][idx] for idx in cluster_idx],
                        [switch_vec[idx] for idx in cluster_idx],
                    'global')).x
                beta_df_cluster = float(v_cluster[0])  
                beta_ds_cluster = float(v_cluster[1])  
                beta_dp_cluster = float(v_cluster[2])  
                
            else:
                beta_df_cluster, beta_ds_cluster, beta_dp_cluster = np.nan, np.nan, np.nan  

            # switch trials
            if len(switch_idx) > 0:
                v_switch = minimize(
                    forage.model_dynamic_phon, 
                    [r1_switch, r2_switch, r3_switch], 
                    args=(
                    [history_vars[2][idx] for idx in switch_idx],  
                    [history_vars[3][idx] for idx in switch_idx],
                    [history_vars[0][idx] for idx in switch_idx],
                    [history_vars[1][idx] for idx in switch_idx],
                    [history_vars[4][idx] for idx in switch_idx],
                    [history_vars[5][idx] for idx in switch_idx],
                    [switch_vec[idx] for idx in switch_idx],
                    'global')).x
                beta_df_switch = float(v_switch[0])  
                beta_ds_switch = float(v_switch[1])  
                beta_dp_switch = float(v_switch[2])  
                
            else:
                beta_df_switch, beta_ds_switch, beta_dp_switch = np.nan, np.nan, np.nan  

            # Compute NLL using all data
            nll, nll_vec = forage.model_dynamic_phon_report(
                [beta_df_cluster, beta_ds_cluster, beta_dp_cluster, beta_df_switch, beta_ds_switch, beta_dp_switch], 
                history_vars[2], history_vars[3], 
                history_vars[0], history_vars[1], 
            ...
            model_results.append((
                beta_df_cluster, beta_ds_cluster,  
...
def run_model(data, model_type, switch_type, domain, speech, dimension, history_vars_list, switch_names_list, switch_vecs_list, alpha=None):
    ...
    for i, (subj, fl_list) in enumerate(tqdm(data)):
        ...
        for i, model in enumerate(model_names): #random and static
            model_dict = dict()
            model_dict['Subject'] = subj
            model_dict['Model'] = model
            model_dict['Beta_Frequency_Cluster'] = model_results[i][0]
            model_dict['Beta_Semantic_Cluster'] = model_results[i][1]
            
            if len(model_results[i]) == 5: #pstatic
                model_dict['Beta_Phonological_Cluster'] = model_results[i][2]

            if len(model_results[i]) == 6: #dynamic
                model_dict['Model'] = model
                model_dict['Beta_Frequency_Switch'] = model_results[i][2]
                model_dict['Beta_Semantic_Switch'] = model_results[i][3]

            if len(model_results[i]) == 8: #pdynamic
                model_dict['Beta_Frequency_Switch'] = model_results[i][2]
                model_dict['Beta_Semantic_Switch'] = model_results[i][3]
                model_dict['Beta_Phonological_Cluster'] = model_results[i][4]
                model_dict['Beta_Phonological_Switch'] = model_results[i][5]
    ...
    column_order = [col for col in forager_results.columns if col != 'Negative_Log_Likelihood_Optimized'] + ['Negative_Log_Likelihood_Optimized']
    forager_results = forager_results[column_order]
