...
def calculate_model(model, history_vars, switch_names, switch_vecs):
  ...
  # Phon-inclusive Unoptimized Model
    model_name.append('forage_prandom_baseline')
    nll_pbaseline, nll_pbaseline_vec = forage.model_static_phon_report(beta = [0,0,0], freql = history_vars[2], freqh = history_vars[3], siml = history_vars[0], simh = history_vars[1], phonl = history_vars[4], phonh = history_vars[5])
    model_results.append((0,0,0, nll_pbaseline, nll_pbaseline_vec))
